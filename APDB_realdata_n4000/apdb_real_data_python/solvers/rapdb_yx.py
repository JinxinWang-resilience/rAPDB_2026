...
def run_rapdb_yx(
    ...
    *,
    ...
    mu: float = 2.0,
    restart_point: Literal["last", "weighted"] = "last",
):
    if restart_point not in {"last", "weighted"}:
        raise ValueError("restart_point must be either 'last' or 'weighted'.")
    ...
    use_weighted_restart = restart_point == "weighted"
    if use_weighted_restart:
    ...
    progress_reporter = make_progress_reporter(
        name=f"rAPDB-yx({mode_name}, mu={mu_f:g})",
        max_iter=max_iter,
        progress=progress,
    )
    while iter_idx < max_iter and float(torch.norm(x - inp.optsol).item()) / norm_optsol > stop_criteria:
        ...
        if iter_idx % int(k_restart) == 0:
            restart_iter_epoch.append(float(iter_idx))
            restart_time_period.append(float(elapsed + time.perf_counter() - t0))

            if use_weighted_restart:
                if tk <= 0.0:
                    raise RuntimeError("Cannot form a weighted restart point with zero total weight.")
                x = x_cum / tk
                y = y_cum / tk
                x_cum.zero_()
                y_cum.zero_()
