...
def run_apdb_c_xy_ada_restart(
    ...
    while iter_idx < max_iter:
        ...
        while True:
            ...
            if ls_debug:
                print(
                    f"[rAPDB-xy-ada][mode={int(non_mono)}][iter={iter_idx}] "
                    f"ls_step={int(inner[iter_idx - 1])} tau={tau:.6e}"
                )

        if iter_idx == 100:
            tg0 = time.perf_counter()
            g_xi_old = smoothed_duality_gap(np_prob, to_numpy(x), to_numpy(y), xi)
        ...
        if iter_idx % 500 == 0 and np.isfinite(g_xi_old):
            tg0 = time.perf_counter()
            g_xi = smoothed_duality_gap(np_prob, to_numpy(x), to_numpy(y), xi)
            if g_xi < g_xi_old * 0.5:
                restart_iter_epoch.append(float(iter_idx))
                restart_oracle.append(float(orc))
    ...
    out["restart_oracle"] = np.asarray(restart_oracle, dtype=np.float64)
    return out
