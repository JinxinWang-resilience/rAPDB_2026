...
def _validate_apdb_settings(*, reg_param: float, mu_values: tuple[float, ...]) -> None:
    if len(mu_values) == 0:
    ...
    if any(not math.isclose(float(v), 2.0, rel_tol=0.0, abs_tol=1e-12) for v in mu_values):
        raise ValueError("mu_values must contain only 2 for the APDB-yx strong-convex splitting.")
    if not math.isclose(float(reg_param), 1.0, rel_tol=0.0, abs_tol=1e-12):
        raise ValueError("reg_param must be 1 for the APDB-yx strong-convex splitting.")
...
def _single_device_run(
    *,
    ...
    if len(egm_tune_grid) == 0:
        raise ValueError("egm_tune_grid must not be empty.")
    _validate_apdb_settings(reg_param=problem.reg_param, mu_values=mu_values)
    ...
    for mu in tuple(float(v) for v in mu_values):
        ...
        for mode in [0, 1]:
            ...
            k_restart = RESTART_CONFIG["rapdb_yx"][mode_name]
            k_restart_ada = RESTART_CONFIG["rapdb_yx_ada"][mode_name]
            ...
            curves[f"rAPDB-yx|{mode_name}|mu={mu_label}"] = {
                ...
                "rel": out_r.rel_err_x,
                "restart_iter": out_r.restart_iter_epoch,
                "restart_time": out_r.restart_time_period,
            }
            ...
            raw[f"restart_iter_rapdb_mu{mu_label}_{mode}"] = out_r.restart_iter_epoch
            raw[f"restart_time_rapdb_mu{mu_label}_{mode}"] = out_r.restart_time_period
            ...
            curves[f"rAPDB-yx-ada|{mode_name}|mu={mu_label}"] = {
                ...
                "restart_iter": out_ra.restart_iter_epoch,
                "restart_time": out_ra.restart_time_period,
            }
            ...
            raw[f"restart_iter_rapdb_ada_mu{mu_label}_{mode}"] = out_ra.restart_iter_epoch
            raw[f"restart_time_rapdb_ada_mu{mu_label}_{mode}"] = out_ra.restart_time_period
...
def run_real_data_experiment(
    *,
    ...
    gpu_curves: dict[str, dict[str, np.ndarray]] | None = None
    gpu_used = False
    ...
    if run_all:
        ...
        gpu_df = cpu_df
        gpu_raw = cpu_raw
        gpu_curves = cpu_curves
        cpu_df = pd.DataFrame(columns=cpu_df.columns)

    plot_png, plot_pdf = plot_cpu_curves(cpu_curves, out, gpu_curves=gpu_curves)
