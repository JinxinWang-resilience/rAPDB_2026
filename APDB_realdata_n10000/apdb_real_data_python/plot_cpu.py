...
def plot_cpu_curves(
    ...
    *,
    ...
    if gpu_curves is not None and not reference_match:
        ax1.set_title("CPU (original MOSEK reference)")
    ...
    ax2.grid(True)
    if gpu_curves is None:
        timing_x_limit = TIME_X_LIMIT
    else:
        max_gpu_time = max(
            (float(np.nanmax(np.asarray(curve["time"], dtype=np.float64)))
             for curve in gpu_curves.values() if len(np.asarray(curve["time"]).reshape(-1))),
            default=0.0,
        )
        max_gpu_time = max(0.0, max_gpu_time)
        timing_x_limit = (0, max(GPU_TIME_X_LIMIT[1], 50 * np.ceil(max_gpu_time * 1.05 / 50)))
    ...
    if gpu_curves is not None and not reference_match:
        ax2.set_title("GPU (recomputed MOSEK reference)")
