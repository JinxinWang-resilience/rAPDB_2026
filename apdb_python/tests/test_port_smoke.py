...
def test_plot_results_both_style_writes_two_files(tmp_path):
    plotting = importlib.import_module("apdb_python.plotting")

    one = [np.array([1.0, 2.0, 3.0], dtype=np.float64)]
    two = [[np.array([1.0, 2.0, 3.0], dtype=np.float64)] for _ in range(2)]
    vals = [np.array([1e0, 1e-1, 1e-2], dtype=np.float64)]
    vals2 = [[np.array([1e0, 1e-1, 1e-2], dtype=np.float64)] for _ in range(2)]
    results = {
        "oracle_egm": one,
        "rel_subopt_egm": vals,
    ...
    assert out[1].name.endswith("_custom.png")
...
def test_plot_style_strict_config_matches_matlab():
    ...
    assert cfg["nonmono_markers"] == [("o", 500, 0), ("^", 700, 0), ("+", 800, 0)]
    assert cfg["infeas_legend_last"] == "rAPDB"
...
def test_plot_style_strict_uses_matlab_like_labels_and_axes():
    ...
    assert lg0 is not None and lg1 is not None
    assert lg0.get_texts()[-1].get_text() == "rAPDB-xy"
    assert lg1.get_texts()[-1].get_text() == "rAPDB"
    assert lg0.get_texts()[0].get_fontsize() == pytest.approx(11.0)
